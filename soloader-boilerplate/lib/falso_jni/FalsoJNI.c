...
jclass FindClass(JNIEnv* env, const char* name) {
    ...
    if (!name) {
        fjni_log_err("[JNI] FindClass(NULL)");
        return NULL;
    }
...
/* Minimal DirectByteBuffer for CTR loadImage → imageLoaded. */
#define FJNI_DBUF_MAGIC 0x464A4442u /* 'FJDB' */
...
jobject NewDirectByteBuffer(JNIEnv* env, void* address, jlong capacity) {
    ...
    if (!address || capacity <= 0)
        return NULL;
    FalsoDirectBuffer *b = (FalsoDirectBuffer *)malloc(sizeof(FalsoDirectBuffer));
    if (!b)
        return NULL;
    b->magic = FJNI_DBUF_MAGIC;
    b->addr = address;
    b->capacity = capacity;
    return (jobject)b;
}
...
void* GetDirectBufferAddress(JNIEnv* env, jobject buf) {
    (void)env;
    FalsoDirectBuffer *b = (FalsoDirectBuffer *)buf;
    if (!b || b->magic != FJNI_DBUF_MAGIC)
        return NULL;
    return b->addr;
}
...
void jni_init() {
    ...
    memset(_jvm, 0, sizeof(struct JNIInvokeInterface));
    ...
    memset(_jni, 0, sizeof(struct JNINativeInterface));
