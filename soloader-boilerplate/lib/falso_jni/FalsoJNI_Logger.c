...
static void fjni_log_to_file(const char *msg) {
...
#define LOG_PRINT \
    LOG_PRINT_TTY \
    fjni_log_to_file(_fjni_log_buffer_2);
...
void _fjni_log_debug(const char *fi, int li, const char *fn, const char* fmt, ...) {
    ...
    /* Never open loader.log for DBG — thousands of JNI lines stall boot. */
    LOG_PRINT_TTY
    LOG_UNLOCK
#endif
}
