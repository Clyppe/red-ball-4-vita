...
void gl_preload() {
    if (!file_exists("ur0:/data/libshacccg.suprx")
        ...
        fatal_error("Error: libshacccg.suprx is not installed.\n"
                    "Install ShaRKBR33D from VitaDB, then reboot.");
    }
    /* Do NOT call vitaGL APIs here — vglInit has not run yet. */
}

void gl_init() {
    /*
     * Leave physically-contiguous RAM free for sceAvPlayer (cutscenes).
     * vglInitExtended(..., phycont_threshold=0) hands ALL phycont to vitaGL,
     * so AvPlayer texture allocs fail and videos stay on a black screen.
     * Matching vitaGL video_playback sample: reserve 32MB phycont.
     */
    vglInitWithCustomThreshold(0, 960, 544,
                               8 * 1024 * 1024,   /* leave 8MB system RAM free */
                               0,                 /* use available CDRAM */
                               32 * 1024 * 1024,  /* leave 32MB PHYCONT for AvPlayer */
                               0,
                               SCE_GXM_MULTISAMPLE_NONE);
#ifdef USE_GLSL_SHADERS
    vglSetSemanticBindingMode(VGL_MODE_POSTPONED);
#endif
    /* Classic CTR uploads RGB/RGBA with tight packing; default align=4 corrupts rows. */
...
/* vitaGL can mishandle GL_RGB uploads; expand to RGBA. */
void glTexImage2D_soloader(GLenum target, GLint level, GLint internalformat,
                           GLsizei width, GLsizei height, GLint border,
                           GLenum format, GLenum type, const GLvoid *pixels) {
    if (pixels && format == GL_RGB && type == GL_UNSIGNED_BYTE &&
    ...
    if (format == GL_RGB && (internalformat == GL_RGB || internalformat == 3))
        ifmt = GL_RGBA;
    glTexImage2D(target, level, ifmt, width, height, border, format, type, pixels);
}
