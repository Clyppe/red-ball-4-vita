...
static float g_last_touch_x[16];
static float g_last_touch_y[16];
...
void poll_touch() {
    ...
    for (int i = 0; i < touch.reportNum; i++) {
        float vx = (float) touch.report[i].x * (float)CTR_PHYS_W / 1920.0f;
        float vy = (float) touch.report[i].y * (float)CTR_PHYS_H / 1088.0f;
        float x, y;
        display_touch_to_game(vx, vy, &x, &y);

        int id = touch.report[i].id & 15;
        int finger_down = 0;
        ...
        if (!finger_down) {
            ...
            float dx = x - g_last_touch_x[id];
            float dy = y - g_last_touch_y[id];
            if (dx * dx + dy * dy >= 1.0f) {
    ...
    for (int i = 0; i < touch_old.reportNum; i++) {
        ...
        if (finger_up == 1) {
            float vx = (float) touch_old.report[i].x * (float)CTR_PHYS_W / 1920.0f;
            float vy = (float) touch_old.report[i].y * (float)CTR_PHYS_H / 1088.0f;
            float x, y;
            display_touch_to_game(vx, vy, &x, &y);
